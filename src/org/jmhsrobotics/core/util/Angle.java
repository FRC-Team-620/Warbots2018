...
public class Angle
{
	 ...
	 */
	public final static Angle ZERO = new Angle(0), RIGHT = new Angle(.25), STRAIGHT = new Angle(.5), INVERSE_RIGHT = new Angle(-.25);
	...
	private static Angle from(double val, double k)
	{
		...
		// This code takes care of all of the weird modulo crap to wrap the angle between -1/2 and 1/2 turns.
		val += .5;
		val -= (int) val;
		if (val < 0) val += 1;
		val -= .5;
	...
	{
		//more weird modulo crap you need not concern yourself with
