...
public class OldTurnTableControlCommand extends CommandModule implements PerpetualCommand, TurnTableController
{
	...
	public void reset()
	{
		...
		if (!fileHandler.isPresent()) 
			DriverStation.reportError("No file handler in robot!! Turntable may take longer to calibrate.", false);
	...
	protected void execute()
	{
		...
		if (currentPosition == null) //Calibrate turn table
		{
			...
			else
			{
				...
				else if (positionEstimate > calibrationRange)
					tableHardware.drive(-BASE_TURN_SPEED);
				else
					calibrationRange *= -CALIBRATION_OVERSHOOT_MULTIPLIER;
			}
